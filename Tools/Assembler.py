...
00000 000 000 ccc 00 - MOV data,RC     : Load RC with data
...
00100 aaa 000 ccc 00 - MOV [RA],RC     : Load RC from the mem addr in RA
00100 aaa 000 000 01 - ENDHP [RA]      : Set the end of the heap in the SP to the mem addr in RA
00101 aaa bbb ccc 00 - MOV [RA,RB],RC  : Load RC from the mem addr given by (RA + RB)
00110 aaa 000 ccc 00 - MOV RA,[RC]     : Store RA to the mem addr in RC
00111 aaa bbb ccc 00 - MOV RA,[RC,RB]  : Store RA to the mem addr given by (RC + RB)
...
# Convert a MOV command to one of the Load/Store commands
def adj_mov_cmd(args):
   if args is None:
      raise SyntaxError()
   ...
   if len(params) == 2:
      if '[' not in args:
         if params[0].strip() in regs.keys():
            return 'MOV'
         else:
            return 'LDI'
      elif '[' in params[0]:
...
# Fail if using a Load/Store command (now obsolete) & also deal with MOV commands
def check_obs_mov(line):
   line = line.split(' ')
   cmd = line[0]
   args = None
   if len(line) > 1:
   ...
   if cmd.startswith('LD') or cmd.startswith('ST'):
      raise SyntaxError("Obsolete Command - Use 'MOV'")
   elif cmd == 'MOV':
      cmd = adj_mov_cmd(args)
...
try:
         ...
         if len(line) > 0:
            ...
            if ':' in line:
               ...
               # Adjust for STR command
                  ...
                  # Deal with obsolete Load/Store commands & the 'MOV' command
                  cmd = check_obs_mov(cmd)

                  # Adjust for double word commands
                  if double_word_cmd(cmd):
                        addr += 1

            else:
               # Deal with obsolete Load/Store commands & the 'MOV' command
               cmd = check_obs_mov(line)
         ...
         for line in asm:
            ...
            if len(line) != 0 and not line.endswith(':'):
               ...
               if line.startswith('STR'):
                  ...
                  # Remove any whitespace before/after any opening brackets (but not for 'set heap' or 'move')
                  if not line.startswith('ENDHP') and not line.startswith('MOV'):
                     line = rem_whitspace(line, '[')
               ...
               # Deal with MOV command - Convert it to one of the Load/Store commands
               if cmd == 'MOV':
                  cmd = adj_mov_cmd(args)
   ...
   if str(se) == 'None':
      ...
      print(f'Syntax error at line: {lineNo} ({se})')
   exit(1)
