        ...
        for var in globals:
            if globals[var][GLOBAL_IS_ARR]:
                ...
                for i, val in enumerate(init_vals):
                    if i == 0:
                        asm.write(f'{var}: word {hex(val)}\n')
                    else:
                        asm.write(f'{var}_{i}: word {hex(val)}\n')
                asm.write(f'\n.= {hex(next_addr)}\n\n')
            else:
...
def processVarUpdate(node):
    ...
    if node.val_node.node_type == N.NODE_NUMBER:
        asm.write(f'   ldi r0,{hex(node.val_node.num)}\n')
    elif node.val_node.node_type == N.NODE_CHAR:
        asm.write(f'   ldi r0,{hex(node.val_node.char)}\n')
    ...
    if node.expr_node is None:
        ...
        if node.node_type == N.NODE_EXPRESSION:
            if node.op is None and node.term_l.node_type == N.NODE_NUMBER:
                ...
                asm.write(f'   ldi r0,{hex(node.term_l.num)}\n')
    ...
    for i in range(nParams):
        ...
        if node.param_vals[i].node_type == N.NODE_IDENTIFIER:
            ...
            if node.param_vals[i].node_type == N.NODE_NUMBER:
                ...
                asm.write(f'   ldi {reg},{hex(num)}\n')
            else:
                char = int(node.param_vals[i].char)
                asm.write(f'   ldi {reg},{hex(char)}\n')
...
def calcInitialValue(node):
    val = calcInitExprVal(node)

    # If 'val' is a 'float' get its 'hex' value
    if isinstance(val, float):
        val = dec_to_hex(str(val))

    return val

def calcInitExprVal(node, sub_expr=False):
    ...
    if node.node_type == N.NODE_NUMBER:
        ...
        if sub_expr and var_type == 'float':
            val = hex_to_dec(val, node)
        ...
        lval = calcInitExprVal(node.term_l, sub_expr=True)
        rval = calcInitExprVal(node.term_r, sub_expr=True)
...
def calcExpression(node, name=None):
    ...
    if node.node_type == N.NODE_NUMBER:
        ...
        asm.write(f'   ldi r0,{hex(node.num)}\n')
        ...
        var_types_used.add(node.type)
        asm.write(f'   ldi r0,{hex(node.char)}\n')
        processUnaryOp(unary_op, node.type)
        asm.write('   push r0\n')
        return
    ...
    if node.term_r.node_type == N.NODE_NUMBER:
        ...
        asm.write(f'   ldi r0,{hex(node.term_r.num)}\n')
        asm.write('   push r0\n')
...
def getPtr(node):
    ...
    if var in locals:
        if var in array_addrs:
            asm.write(f'   ldi r0,{hex(array_addrs[var])}\n')
        else:
        ...
        asm.write(f'   ldi r0,{hex(globals[var][GLOBAL_ADDR])}\n')
